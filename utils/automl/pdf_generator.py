...
class AutoMLPDFReportGenerator:
    ...
    def generate_report(
        ...
        # Embed Data Visualization Chart if exists in dictionary
        if "data_visualization" in visualizations_dict:
            try:
                tf = tempfile.NamedTemporaryFile(suffix=".png", delete=False)
                tf.write(visualizations_dict["data_visualization"])
                tf.close()
                ...
                story.append(KeepTogether([
                    Paragraph(cls._process_text("Dataset Feature Analysis" if not is_arabic else "التحليل البصري لميزات البيانات", is_arabic, font_registered), bold_body_style),
                    Image(tf.name, width=320, height=220),
                    Spacer(1, 10)
                ]))
            except Exception as data_chart_err:
                print(f"Failed to embed data visualization chart in PDF: {data_chart_err}")
        ...
        # Embed ROC Curve / Prediction vs Actual plot
        roc_key = "roc_curve" if is_cls else "pred_vs_actual"
        roc_title = ("Receiver Operating Characteristic (ROC) Curve" if is_cls else "Prediction vs. Actual Comparison") if not is_arabic else ("منحنى خصائص تشغيل المستقبل (ROC)" if is_cls else "مقارنة القيم المتوقعة بالقيم الحقيقية")
        
        if roc_key in visualizations_dict:
            try:
                tf = tempfile.NamedTemporaryFile(suffix=".png", delete=False)
                tf.write(visualizations_dict[roc_key])
                tf.close()
                temp_files.append(tf.name)
                ...
                story.append(KeepTogether([
                    Paragraph(cls._process_text(roc_title, is_arabic, font_registered), bold_body_style),
                    Image(tf.name, width=320, height=220),
                    Spacer(1, 10)
                ]))
            except Exception as roc_err:
                print(f"Failed to embed ROC/pred-vs-actual chart in PDF: {roc_err}")
        ...
        if feature_importance:
            ...
            # Embed Feature Importance Chart if exists in dictionary
            if "feature_importance" in visualizations_dict:
                try:
                    tf = tempfile.NamedTemporaryFile(suffix=".png", delete=False)
                    tf.write(visualizations_dict["feature_importance"])
                    tf.close()
                    temp_files.append(tf.name)
                    ...
                    story.append(KeepTogether([
                        Paragraph(cls._process_text("Feature Importance Profile" if not is_arabic else "مخطط أهمية الخصائص", is_arabic, font_registered), bold_body_style),
                        Image(tf.name, width=320, height=220),
                        Spacer(1, 10)
                    ]))
                except Exception as imp_chart_err:
                    print(f"Failed to embed feature importance chart in PDF: {imp_chart_err}")
