"""
core/automl/llm_triage.py — SOL Agentic Model Router
Identical to source — no Streamlit dependency.
"""
import json
import logging
import time
...
OUTPUT_JSON_SCHEMA = {
    ...
    "schema": {
        ...
        "properties": {
            "approved_models": {
                "type": "array",
                "items": {"type": "string", "enum": ["LogisticRegression","KNN","SVM","DecisionTree","RandomForest","ExtraTrees","GradientBoosting","XGBoost","LightGBM","CatBoost"]}
            },
            "rejected_models": {
                "type": "array",
                "items": {"type": "string", "enum": ["LogisticRegression","KNN","SVM","DecisionTree","RandomForest","ExtraTrees","GradientBoosting","XGBoost","LightGBM","CatBoost"]}
            ...
            "reasoning": {
                ...
                "required": ["LogisticRegression","KNN","SVM","DecisionTree","RandomForest","ExtraTrees","GradientBoosting","XGBoost","LightGBM","CatBoost"],
...
def get_llm_model_triage(df: pd.DataFrame, target_col: str, task_type: str, api_key: Optional[str]) -> Dict[str, Any]:
    all_candidates = ["LogisticRegression","KNN","SVM","DecisionTree","RandomForest","ExtraTrees","GradientBoosting","XGBoost","LightGBM","CatBoost"]
    ...
    if len(df) > 20000 or len(df.columns) > 50:
        fallback_approved = [m for m in fallback_approved if m != "KNN"]
    if len(df) > 15000:
        fallback_approved = [m for m in fallback_approved if m != "SVM"]
    default_res = {
        "approved_models": fallback_approved,
        "rejected_models": [m for m in all_candidates if m not in fallback_approved],
        "reasoning": {m: "Approved by default heuristic." if m in fallback_approved else "Rejected by default heuristic." for m in all_candidates}
    }
    if not api_key:
        return default_res
    try:
    ...
    num_cat = len(df.select_dtypes(exclude=["number"]).columns)
    missing_pct = round(df.isnull().mean().mean() * 100, 2)
    high_card = [f"{c} (cardinality: {df[c].nunique()})" for c in df.select_dtypes(exclude=["number"]).columns if c != target_col and df[c].nunique() > 50]
    minority_pct = 0.0
...
- Rows: {num_rows}
- Columns: {num_cols}
- Numeric Columns Count: {num_numeric}
- Categorical Columns Count: {num_cat}
- Class Balance (Minority Class %): {minority_pct}%
- Missing Value Percentage: {missing_pct}%
- High-Cardinality Categorical Features (cardinality > 50): {high_card}
"""
    try:
        ...
        if not result.get("approved_models"):
            return default_res
        for m in all_candidates:
            if m not in result.get("reasoning", {}):
                result.setdefault("reasoning", {})[m] = "Approved." if m in result["approved_models"] else "Rejected."
        return result
    except Exception as e:
        logger.error("LLM model triage failed: %s. Returning defaults.", e)
        return default_res
