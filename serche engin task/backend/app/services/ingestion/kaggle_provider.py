...
from backend.app.services.ingestion.base import BaseDatasetProvider, StandardizedDataset
...
class KaggleProvider(BaseDatasetProvider):
    ...
    def _setup_credentials(self) -> bool:
        ...
        if not api_key:
            logger.warning("KAGGLE_API_KEY is not defined in environment.")
            return False
        
        os.environ["KAGGLE_USERNAME"] = username or "shalllaby"
        ...
        try:
            ...
            for ds in datasets_raw[:limit]:
                ...
                standard_list.append(StandardizedDataset(
                    ...
                    column_count=getattr(ds, 'columnCount', 10) or 10,
    ...
    def _get_fallback_seeds(self, query: str) -> List[StandardizedDataset]:
        ...
        seeds = [
            ...
            StandardizedDataset(
                ...
                description="Patient clinical metrics database detailing chest pain types, cholesterol, blood pressure, and binary heart disease labels. Recommended for beginners practicing classification models.",
